...
{
    /// <summary>
    /// "None" event used not to be an individual event but a flag to composite with other states
    /// to represent a composite state.
    /// <example>
    /// <c>var @event = NodeEvents.None | NodeEvents.Start</c> represents an event to start the execution.
    /// But it could be also written in form like <c>var @event  = NodeEvents.Start</c>, those two have totally same effect.
    /// So you haven't to use it implicitly.
    /// </example>
    /// </summary>
    None = 0,
